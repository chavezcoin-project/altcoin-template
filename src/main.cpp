...
#if defined(NDEBUG)
# error "Bolivarcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8a70b5d892df94550e93031bf512cf2495571feac2a517694c739f6b345fe45e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bolivarcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bolivarcoin Signed Message:\n";
...
{
    // Bolivarcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bolivarcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bolivarcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8a70b5d892df94550e93031bf512cf2495571feac2a517694c739f6b345fe45e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "A new economy just to begin in Venezuela!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x9c29976b1d027f72a670129aadf6adee3d702f9a14d3f45bc358de5c720cb7cf"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Bolivarcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BolivarcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BolivarcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BolivarcoinMiner(CWallet *pwallet)
{
    printf("BolivarcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bolivarcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BolivarcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BolivarcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BolivarcoinMiner, pwallet));
}
